...
ga_dict = model.state_dict(keep_vars=True)
# for key in ga_dict.keys():
#     if key.find("weight") != -1:
#         print(ga_dict[key].grad)

s_adv_grad = adv_grad.sign()
# ptb_amp_choices = [0.001, 0.002, 0.003, 0.004, 0.005, 0.006, 0.007, 0.01]
ptb_amp_choices = np.arange(0.001, 0.05, 0.001)
robust_dict = {}
...
for ptb_amp in ptb_amp_choices:
    ...
    for noise_index, key in enumerate(state_dict.keys()):
        if key.find("weight") != -1:
            pState[key] = pState[key].data + ga_dict[key].grad.data.sign() * ptb_amp
...
torch.save(robust_dict, "gradient_accent_dict_9874.pt")