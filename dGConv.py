...
if __name__ == "__main__":
    ...
    for _ in tqdm.tqdm(range(100)):
        ...
        # for _ in range(nRuns):
            ...
            dModel = Net()
            dModel.load_state_dict(state_dict)
            dState = pModel.state_dict()
            for noise_index, key in enumerate(state_dict.keys()):
                if key.find("weight") != -1:
                    ...
                    dState[key] = dState[key].data - sampled_noise
            pModel.load_state_dict(pState)
            pModel.to(device)
            dModel.load_state_dict(dState)
            dModel.to(device)
            ...
            with torch.no_grad():
                for data in testloader:
                    ...
                    _, predicted = torch.max(outputs.data, 1)
                    dOutput.append(outputs.detach().cpu().numpy())
                    break
        ...
        for j in range(1):
            ...
            alpha.append(np.std(shoot))
            shoot = []
            for i in range(len(dOutput)):
                shoot.append(dOutput[i][0][j] - GT[0][j])
            beta.append(np.std(shoot))
            doot = []
            for i in range(len(pOutput)//2):
                doot.append(pOutput[i][0][j] - GT[0][j])
                doot.append(dOutput[i][0][j] - GT[0][j])
            theD.append(np.std(doot))
    
    print(np.abs(theTruth - np.array(alpha)).mean(), np.std(alpha))
    print(np.abs(theTruth - np.array(beta)).mean(), np.std(beta))
    print(np.abs(theTruth - np.array(theD)).mean(), np.std(theD))
