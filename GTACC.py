...
GT = np.load(os.path.join(path, "GT.npy"))
a = np.load(os.path.join(path, f"output{0}_00.npz"))["arr_0"]
CC = np.argmax(a[0], axis=1)

patterns = ["1.0", "5e-1", "1e-1", "4e-2", "1e-2"]

all_list = []
for pattern in patterns:
    # pattern = "1.0"
    a = np.load(os.path.join(path, f"output{pattern}_00.npz"))["arr_0"]
    ...
    for i in range(len(a)):
        ...
        acc_list.append(acc)
        crr_list.append(crr)
    print(f"acc: {np.mean(acc_list)}, same: {np.mean(crr_list)}")
    
    all_list.append((acc_list, crr_list))
    continue
...
torch.save(all_list, "accvscrr.pt")